"""Inventory tests — Selenium edition, Allure-native, DDT-driven."""
...
@pytest.fixture
def inventory(driver) -> InventoryPage:
    """Log in and return InventoryPage with a clean cart."""
...
def test_inventory_display(inventory: InventoryPage, tc: dict):
    ...
    if tc.get("id") == "TC-INV-003":
        with step("Verify each product image has valid src"):
            ...
            items = inventory.product_items
            for i in range(len(items)):
                img = items[i].find_element(By.CSS_SELECTOR, "img.inventory_item_img")
